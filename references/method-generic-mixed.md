...
- `FACT`: what the image plainly contains
- `INFERENCE`: strongest route guess backed by visible evidence
- `HYPOTHESIS`: weaker route guesses or context assumptions that still need confirmation
...
1. `Describe`: what is plainly visible
2. `Structure`: how the image organizes attention
3. `Meaning`: what it seems to communicate or evoke
4. `Judgment`: what works, what fails, what route would unlock deeper critique
